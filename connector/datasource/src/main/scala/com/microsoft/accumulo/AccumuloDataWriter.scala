...
class AccumuloDataWriter(tableName: String,
                         schema: StructType,
                         mode: SaveMode,
      ...
      cf.dataType match {
        ...
        case _ =>
          ...
          (mutation: Mutation, record: InternalRow) => {
            ...
            // not using the fluent interface to provide backward compatibility
  ...
  def write(record: InternalRow): Unit = {
    ...
    // skip if the rowKey is null or ignore flag is set
    if (rowKeyRaw != null && !ignore) {
